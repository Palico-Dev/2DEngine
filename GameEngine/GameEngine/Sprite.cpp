...
void Sprite::Serialize(json::JSON& j)
{
	FileManager::JsonWriteString(j, "name", spriteName);
	FileManager::JsonWriteInt(j, "layer", layer);
	FileManager::JsonWriteString(j, "asset", textureAsset->GetFileName());
	FileManager::JsonWriteVec2(j, "size", size);
	FileManager::JsonWriteVec2(j, "offset", offset);
	FileManager::JsonWriteString(j, "flip", flipString);
	FileManager::JsonWriteFloat(j, "rotation", rotation);
	FileManager::JsonWriteColor(j, "color", color);
}
...
void Sprite::Load(json::JSON& _document)
{
	...
	{
		assetName = FileManager::JsonReadString(_document, "asset");
		textureAsset = AssetManager::Instance().GetAsset<TextureAsset>(assetName);
	}

	size = FileManager::JsonReadVec2(_document, "size");
	offset = FileManager::JsonReadVec2(_document, "offset");

	flipString = FileManager::JsonReadString(_document, "flip");
